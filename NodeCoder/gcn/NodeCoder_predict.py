...
class NodeCoder_Predictor(object):
    ...
    def do_prediction(self, graph_data, cluster):
        ...
        for i in range(0, self.TaskNum):
            target_0 = np.array(graph_data.target[:, i])
    ...
    def test_metrics_perprotein(self):
        ...
        for i in range(0, max(protein_ID)+1):
            ...
            self.BalancedAcc.append(metrics.balanced_accuracy_score(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions[0, Protein[i]].cpu().reshape(-1, 1)))
            self.F1score.append(metrics.f1_score(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions[0, Protein[i]].cpu().reshape(-1, 1), pos_label=1,zero_division=1))
            self.MCC = metrics.matthews_corrcoef(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions[0, Protein[i]].cpu().reshape(-1, 1))
            try:
                self.Precision.append(metrics.precision_score(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions[0, Protein[i]].cpu().reshape(-1, 1), zero_division=1)) #average="micro"
            except ValueError:
                self.Precision.append(0)
            try:
                self.Recall.append(metrics.recall_score(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions[0, Protein[i]].cpu().reshape(-1, 1), zero_division=1)) #average="micro", macro:This does not take label imbalance into account.
            except ValueError:
                self.Recall.append(0)
            try:
               self.ROCAUC.append(metrics.roc_auc_score(self.protein_targets[0, Protein[i]].cpu().reshape(-1, 1), self.protein_predictions_prob[0, Protein[i]].cpu().reshape(-1, 1)))
            except ValueError:
               self.ROCAUC.append(0)
            try:
               ...
               self.PRAUC.append(metrics.auc(recall, precision))
            except ValueError:
               self.PRAUC.append(0)
